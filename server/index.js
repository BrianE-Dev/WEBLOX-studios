...
async function handler(req, res) {
  ...
  if (origin && origin !== allowedOrigin) return send(res, 403, { error: 'Origin not allowed' })
  ...
  if (req.method === 'POST' && url.pathname === '/api/staff/sync') {
    ...
    for (const entry of entries) {
      ...
      if (email) staffDirectory.set(email, { email, name: String(entry.name || '').trim(), role: String(entry.role || '').trim() })
    }
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/register') {
    ...
    if (!email || !name || !role || password.length < 10) return send(res, 400, { error: 'Name, email, role, and a password of at least 10 characters are required.' })
    const credentials = await passwordRecord(password)
    const account = { id: createHash('sha256').update(email).digest('hex').slice(0, 16), email, name, role, ...credentials, createdAt: new Date().toISOString() }
    if (!await store.createAccount(account)) return send(res, 409, { error: 'An account already exists for this email.' })
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/login') {
    ...
    const actual = await passwordRecord(password, Buffer.from(account.salt, 'hex'))
    const expectedHash = Buffer.from(account.hash, 'hex')
    const actualHash = Buffer.from(actual.hash, 'hex')
    if (expectedHash.length !== actualHash.length || !timingSafeEqual(expectedHash, actualHash)) return send(res, 401, { error: 'Email or password is incorrect.' })
    const token = randomBytes(32).toString('hex')
    sessions.set(token, { account, expiresAt: Date.now() + sessionTtl })
    return send(res, 200, { account: publicAccount(account) }, { 'set-cookie': `${cookieName}=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${sessionTtl / 1000}` })
  }

  if (req.method === 'GET' && url.pathname === '/api/auth/session') {
    const token = parseCookies(req.headers.cookie)[cookieName]
    const session = token && sessions.get(token)
    if (!session || session.expiresAt < Date.now()) return send(res, 401, { error: 'Not signed in.' })
...
}).listen(port, () => console.log(`WEBLOX auth API listening on http://localhost:${port}`))
