...
The API can be deployed as a Node web service from this repository's root with build command `npm install` and start command `npm run server`. Configure `DATABASE_URL` with the hosted PostgreSQL connection string, `APP_ORIGIN` with the exact public frontend origin, and `NODE_ENV=production`. The service listens on the platform-provided `PORT` when available. For Render, keep the API and database in the same region and use the database's internal connection URL in the service settings. The Vercel function in `api/[...path].js` proxies browser `/api/...` requests to the API. Set the server-side Vercel variable `API_ORIGIN` to the Render API origin and keep the Render API's `APP_ORIGIN` set to the exact Vercel frontend origin. Browser requests stay on the Vercel origin so the `HttpOnly; SameSite=Lax` session cookie remains first-party.
